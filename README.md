...
**矢印・幾何学図形・星・単位記号・囲み数字・罫線素片が出ない。** 置換字形（◆）になる。

    → ← ↑ ↓ ★ ☆ ● ○ ◆ ◇ ■ □ ▲ △ ℃ ∞ ≠ ① ② ─ │ ♪

web小説では `◇◇◇` の場面区切りや `→` が頻出するので、実用上そこそこ効く。

原因は配信元の収録リストの生成漏れ。リストは JIS X 0213 を名乗っているが、実際は
ASCII・Latin-1・一般句読点・CJK記号・かな・半角全角形と、Unihan の属性から拾った
**漢字だけ**で、JIS の1区・2区にある非漢字の記号がどのブロックからも拾われていない。
**フォント側（TTF）にはこれらの字形が入っている**ので、収録範囲を広げて焼き直せば出る。

一般句読点（U+2000-206F）は全部入っているので、`※` `…` `‥` `—` `―` は出る。
ここを取り違えないこと。

**別の落とし穴として、収録リストにあるのに元フォントに無い字は「空白」になる。**
BIZ UDGothic の実測では 13,474 字の要求のうち **3,227 字が元フォントに存在しない**。
◆ と違って気づきにくい。

対処は2つ。

- **自分で焼き直す**（推奨）。原文を書き換えずに済み、変換を通さない EPUB にも効く。
  自分の端末で使うだけなら再配布にあたらない。手順は
  [フォント生成CLIの仕様](./docs/prompts/build-jp-font-cli.md) に、実地で通した
  コマンドごと書いてある
- **EPUB 変換時に置換する**。他人に渡すファイルを作るときはこちら。方針と、端末で
  出せる文字の一覧は
  [日本語フォントで出せる文字と、変換側での対応方針](./docs/japanese-font-coverage.md)
