...
void WifiSelectionActivity::startWifiScan(const bool autoScan) {
  {
    // Scan completion and rendering run on different cores. Publish the new
    // state while holding the render mutex, then finish the first e-ink paint
    // before starting the radio. This avoids overlapping the X4 Pro's initial
    // full refresh with WiFi scan startup and gives the result path a stable
    // SCANNING screen to replace.
  ...
  if (requestUpdateAndWait() != RequestUpdateResult::Rendered) {
    LOG_ERR("WIFI", "Unable to render scanning screen; scan not started");
    {
      RenderLock lock(*this);
      autoConnecting = false;
      manualNetworkListRequested = false;
