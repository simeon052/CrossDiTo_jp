...
void SettingsActivity::openScreenMarginPicker(const SettingInfo& setting) {
  ...
  startActivityForResult(
      makeUniqueNoThrow<OptionSelectionActivity>(renderer, mappedInput, "SettingsValueSelect", selectedSetting.nameId,
...
void SettingsActivity::applyUiSettingChange(uint8_t CrossPointSettings::* valuePtr) {
  ...
  // The render task reads these fields independently. Replace the theme and
  // font metrics under its mutex so it cannot observe a half-updated set.
  {
    RenderLock lock(*this);
    if (themeChanged) {
