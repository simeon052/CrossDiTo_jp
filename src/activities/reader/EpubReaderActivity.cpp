...
constexpr char WHOLE_BOOK_PAGE_INDEX_FILE_NAME[] = "/book_pages.bin";
constexpr char WHOLE_BOOK_PAGE_INDEX_TMP_SUFFIX[] = ".tmp";
constexpr char WHOLE_BOOK_PAGE_INDEX_BACKUP_SUFFIX[] = ".bak";
constexpr uint32_t WHOLE_BOOK_PAGE_INDEX_MAGIC = 0x49504243U;  // "CBPI" on disk
constexpr uint8_t WHOLE_BOOK_PAGE_INDEX_VERSION = 2;
constexpr size_t WHOLE_BOOK_PAGE_INDEX_HEADER_SIZE = sizeof(WHOLE_BOOK_PAGE_INDEX_MAGIC) +
                                                     sizeof(WHOLE_BOOK_PAGE_INDEX_VERSION) + sizeof(uint32_t) +
...
constexpr uint32_t IDLE_PAGE_PREFETCH_MIN_FREE = 96U * 1024U;
constexpr uint32_t IDLE_PAGE_PREFETCH_MIN_MAX_ALLOC = 48U * 1024U;
constexpr uint32_t RETAINED_PAGE_MIN_FREE = 64U * 1024U;
constexpr uint32_t RETAINED_PAGE_MIN_MAX_ALLOC = 32U * 1024U;
...
void recoverWholeBookPageIndexBackup(const std::string& indexPath) {
  const std::string backupPath = indexPath + WHOLE_BOOK_PAGE_INDEX_BACKUP_SUFFIX;
  if (Storage.exists(indexPath.c_str())) {
    if (Storage.exists(backupPath.c_str()) && !Storage.remove(backupPath.c_str())) {
      LOG_ERR("ERS", "Failed to remove stale whole-book page-index backup");
    }
    return;
  }
  if (Storage.exists(backupPath.c_str()) && !Storage.rename(backupPath.c_str(), indexPath.c_str())) {
    LOG_ERR("ERS", "Failed to recover whole-book page-index backup");
  }
}

bool promoteWholeBookPageIndex(const std::string& tmpPath, const std::string& indexPath) {
  const std::string backupPath = indexPath + WHOLE_BOOK_PAGE_INDEX_BACKUP_SUFFIX;
  if (Storage.exists(backupPath.c_str()) && !Storage.remove(backupPath.c_str())) {
    LOG_ERR("ERS", "Failed to remove stale whole-book page-index backup before promotion");
    return false;
  }

  const bool hadIndex = Storage.exists(indexPath.c_str());
  if (hadIndex && !Storage.rename(indexPath.c_str(), backupPath.c_str())) {
    LOG_ERR("ERS", "Failed to back up whole-book page index");
    return false;
  }
  if (Storage.rename(tmpPath.c_str(), indexPath.c_str())) {
    if (hadIndex && Storage.exists(backupPath.c_str()) && !Storage.remove(backupPath.c_str())) {
      LOG_ERR("ERS", "Failed to remove promoted whole-book page-index backup");
    }
    return true;
  }

  LOG_ERR("ERS", "Failed to promote whole-book page index");
  if (hadIndex && !Storage.rename(backupPath.c_str(), indexPath.c_str())) {
    LOG_ERR("ERS", "Failed to restore whole-book page-index backup");
  }
  return false;
}
...
void EpubReaderActivity::onEnter() {
  ...
  // Save the resume target now, but defer recent-books JSON parsing/pruning
  // until after the first page is visible.
  APP_STATE.openEpubPath = epub->getPath();
  APP_STATE.saveToFile();
  pendingRecentBookUpdate = true;
...
void EpubReaderActivity::onExit() {
  ...
  // A cooperative whole-book build can retain a parser between loop ticks.
  // Suspend it before any other activity starts using the SD card. Completed
  // section work remains as a partial cache and is reused on the next open.
  resetWholeBookPaginationBuild(true);
  clearPrefetchedPage();
  ...
  CLIPPINGS.unload();
  resetSection();
  // Section owns the only possible open pagination handle. Release it before
  // the deferred recent-books JSON read/write touches the X4 Pro SD card.
  commitPendingRecentBookUpdate();
...
void EpubReaderActivity::openReaderMenu() {
  ...
  bool previewActive = false;
  {
    ...
    if (!previewActive && section && section->currentPage >= 0) {
      pendingMenuOrigin = SavedPosition{currentSpineIndex, section->currentPage};
    }
  }
  ...
  startActivityForResult(
      makeUniqueNoThrow<EpubReaderMenuActivity>(
          ...
          bookSettings.hasDictionaryFontOverride, saveDictionaryFontForBookReader, this, hasPreviousReadingPosition()),
      [this](const ActivityResult& result) {
        if (const auto* clipping = std::get_if<ClippingJumpResult>(&result.data)) {
          ...
          if (clipping->settingsChanged) {
            ...
            resetSection();  // Force re-layout with changed reader settings
          }
...
void EpubReaderActivity::returnToPreviousReadingPosition() {
  ...
  if (footnoteDepth > 0) {
    ...
    LOG_DBG("ERS", "Returning to previous reading position: spine=%d page=%d", position.spineIndex,
            position.pageNumber);
...
void EpubReaderActivity::clearPrefetchedPage() {
  ...
  prefetchedPageSection = nullptr;
  prefetchedPageSpine = -1;
  prefetchedPageNumber = -1;
  prefetchedPageFontId = 0;
  prefetchedPageLayoutSignature = 0;
}
...
std::unique_ptr<Page> EpubReaderActivity::takePrefetchedPage(const int pageNumber, const int fontId) {
  if (!prefetchedPage) return nullptr;
  if (prefetchedPageSection != section.get() || prefetchedPageSpine != currentSpineIndex ||
      prefetchedPageFontId != fontId || prefetchedPageLayoutSignature != activeSectionLayoutSignature) {
    clearPrefetchedPage();
    return nullptr;
  }
  if (prefetchedPageNumber == pageNumber) {
    auto page = std::move(prefetchedPage);
    clearPrefetchedPage();
    return page;
  }
  // A re-render of the current page can keep its immediate successor. Any
  // other navigation makes this speculative page irrelevant.
  if (pageNumber + 1 != prefetchedPageNumber) clearPrefetchedPage();
  return nullptr;
}

void EpubReaderActivity::commitPendingRecentBookUpdate() {
  if (!pendingRecentBookUpdate || !epub) return;
  pendingRecentBookUpdate = false;
...
void EpubReaderActivity::idlePrewarmNextPage() {
  ...
  if (!MemoryBudget::hasHeap(heap, IDLE_PAGE_PREFETCH_MIN_FREE, IDLE_PAGE_PREFETCH_MIN_MAX_ALLOC)) {
    return;
  }
  ...
  const auto retainedHeap = MemoryBudget::snapshot();
  if (!MemoryBudget::hasHeap(retainedHeap, RETAINED_PAGE_MIN_FREE, RETAINED_PAGE_MIN_MAX_ALLOC)) {
    LOG_DBG("ERS", "Discarding idle page prefetch under memory pressure");
    return;
  }
  clearPrefetchedPage();
  prefetchedPage = std::move(page);
  prefetchedPageSection = section.get();
  prefetchedPageSpine = currentSpineIndex;
  prefetchedPageNumber = nextPage;
  prefetchedPageFontId = renderFontId;
  prefetchedPageLayoutSignature = activeSectionLayoutSignature;
  LOG_DBG("ERS", "Idle page prefetch: spine=%d page=%d in %lums", currentSpineIndex, nextPage, millis() - startedAt);
}

unsigned long EpubReaderActivity::nextLoopWakeDelayMs() const {
  constexpr unsigned long IDLE_READER_HEARTBEAT_MS = 1000UL;
  constexpr unsigned long FEEDBACK_TIMEOUT_MS = 1000UL;
  unsigned long delayMs = IDLE_READER_HEARTBEAT_MS;
  const unsigned long now = millis();

  auto reduceForElapsedTimer = [&](const bool active, const unsigned long startedAt, const unsigned long durationMs) {
    if (!active) return;
    const unsigned long elapsed = now - startedAt;
    const unsigned long remaining = elapsed >= durationMs ? 0UL : durationMs - elapsed;
    if (remaining < delayMs) delayMs = remaining;
  };

  reduceForElapsedTimer(automaticPageTurnActive, lastPageTurnTime, pageTurnDuration);
  reduceForElapsedTimer(pendingBookmarkFeedback, bookmarkFeedbackShowTime, FEEDBACK_TIMEOUT_MS);
  reduceForElapsedTimer(pendingCompletedFeedback, completedFeedbackShowTime, FEEDBACK_TIMEOUT_MS);
  reduceForElapsedTimer(pendingTiltPageTurnFeedback, tiltPageTurnFeedbackShowTime, FEEDBACK_TIMEOUT_MS);
  reduceForElapsedTimer(pendingRenderModeToast || pendingSafeModeToast, renderModeToastShowTime, RENDER_MODE_TOAST_MS);
  return delayMs;
}
...
void EpubReaderActivity::loop() {
  ...
  if (!userInputPending && pendingRecentBookUpdate && lastRenderCompleteMs != 0 && section && !section->isBuilding() &&
      !RenderLock::peek() && millis() - lastRenderCompleteMs >= IDLE_SD_FONT_PREWARM_DELAY_MS) {
    commitPendingRecentBookUpdate();
  }
  ...
  const bool completeCurrentSpineForWholeBook = SETTINGS.indexingMethod == CrossPointSettings::INDEXING_FULL_BOOK &&
                                                wholeBookPaginationBuildState != WholeBookPaginationBuildState::Idle &&
  ...
  if (!backgroundBuildYieldForInput.load(std::memory_order_relaxed) && section && !section->isBuilding() &&
      ...
      (completeCurrentSpineForWholeBook ||
  ...
  if (!backgroundBuildYieldForInput.load(std::memory_order_relaxed) && sectionBuildWantsTick() && !RenderLock::peek() &&
    ...
    if (section && section->isBuilding() && (section->isPartial() || section->activeBuildHasCaughtReadablePages()) &&
      ...
      if (SETTINGS.indexingMethod == CrossPointSettings::INDEXING_FULL_BOOK && section->isBuilding()) {
        // Full-book mode keeps laying out the active spine beyond the normal
        // look-ahead window. Close both SD handles between chunks so a page
        // turn can read immediately on the X4 Pro's single-reader storage.
        section->releaseBuildFile();
      }
  ...
  if (completionPromptQueued) {
    ...
    startActivityForResult(
        makeUniqueNoThrow<ConfirmationActivity>(renderer, mappedInput, tr(STR_MARK_FINISHED_PROMPT_TITLE),
...
bool EpubReaderActivity::jumpToPercent(int percent) {
  pageLoadRetryCount = 0;
  if (!epub) {
    return false;
  }
  ...
  float targetSpineProgress =
      (spineSize == 0) ? 0.0f : static_cast<float>(targetSize - prevCumulative) / static_cast<float>(spineSize);
  if (targetSpineProgress < 0.0f) {
    targetSpineProgress = 0.0f;
  } else if (targetSpineProgress > 1.0f) {
    targetSpineProgress = 1.0f;
  }
  ...
  switch (action) {
      ...
      startActivityForResult(
          ...
          [this](const ActivityResult& result) {
            if (!result.isCancelled) {
              ...
              const int previousDepth = footnoteDepth;
              if (navigateToHref(footnoteResult.href, true)) {
                if (footnoteDepth == previousDepth) {
                  rememberPendingMenuOrigin();
                } else {
                  pendingMenuOrigin.reset();
                }
                ...
                pendingMenuOrigin.reset();
                resumeReadingPaceTimer("footnote_jump_failed");
              }
            } else {
              pendingMenuOrigin.reset();
    ...
    case EpubReaderMenuActivity::MenuAction::GO_TO_PERCENT: {
      ...
      startActivityForResult(
          ...
          [this](const ActivityResult& result) {
            if (!result.isCancelled) {
              if (jumpToPercent(std::get<PercentResult>(result.data).percent)) {
                rememberPendingMenuOrigin();
              } else {
                pendingMenuOrigin.reset();
                resumeReadingPaceTimer("percent_jump_failed");
                requestUpdate();
              }
            } else {
              pendingMenuOrigin.reset();
      ...
      if (section && section->currentPage >= 0 && section->currentPage < section->pageCount) {
        ...
        if (p) {
          ...
          if (!fullText.empty()) {
            ...
            startActivityForResult(
                makeUniqueNoThrow<QrDisplayActivity>(renderer, mappedInput, fullText),
    ...
    case EpubReaderMenuActivity::MenuAction::LOOKUP_HISTORY: {
      ...
      const BookReaderSettingsData bookSettings = loadBookReaderSettingsFile(epub->getCachePath());
      startActivityForResult(makeUniqueNoThrow<LookedUpWordsActivity>(renderer, mappedInput, epub->getCachePath(),
    ...
    case EpubReaderMenuActivity::MenuAction::SET_BOOK_DICTIONARY: {
      ...
      startActivityForResult(makeUniqueNoThrow<DictionarySelectActivity>(renderer, mappedInput, epub->getCachePath()),
                             [this](const ActivityResult&) {
    ...
    case EpubReaderMenuActivity::MenuAction::DELETE_STATS: {
      ...
      startActivityForResult(makeUniqueNoThrow<ConfirmationActivity>(renderer, mappedInput,
    ...
    case EpubReaderMenuActivity::MenuAction::READING_STATS: {
      ...
      if (hasSyncedStats) {
        startActivityForResult(
            makeUniqueNoThrow<BookStatsActivity>(renderer, mappedInput, epub->getTitle(), epub->getCachePath(),
    ...
    case EpubReaderMenuActivity::MenuAction::NEARBY_POSITION_SYNC: {
      ...
      activityManager.replaceActivity(makeUniqueNoThrow<NearbyBookPositionSyncActivity>(
  ...
  switch (action) {
    ...
    case CrossPointSettings::LONG_MENU_SYNC_PROGRESS:
      if (KOREADER_STORE.hasCredentials()) {
        ...
        startActivityForResult(makeUniqueNoThrow<KOReaderSettingsActivity>(renderer, mappedInput),
                               [this](const ActivityResult&) {
...
void EpubReaderActivity::render(RenderLock&& lock) {
  ...
  yieldWholeBookPaginationSpineToReader();
  ...
  if (!section) {
    ...
    // Full Book is now cooperative: render the requested page first, then let
    // loop() finish this and the remaining spines one page at a time.
    ...
    const bool wantsWholeBookPageNumbers =
        SETTINGS.statusBarChapterPageCount == CrossPointSettings::STATUS_BAR_PAGE_COUNT::PAGE_COUNT_BOOK;
    if (!buildingFootnotePreview &&
        (SETTINGS.indexingMethod == CrossPointSettings::INDEXING_FULL_BOOK || wantsWholeBookPageNumbers)) {
      armWholeBookPagination(viewportWidth, viewportHeight);
    }
...

  {
    ...
    const int renderFontId = activeSectionFontId != 0 ? activeSectionFontId : SETTINGS.getReaderFontId();
    auto p = takePrefetchedPage(section->currentPage, renderFontId);
    ...
    if (!p) {
      ...
      if (pageLoadRetryCount <= MAX_PAGE_LOAD_RETRIES) {
        ...
        section->clearCache();
        invalidateWholeBookPaginationIndex();
        resetSection();
        requestUpdate();
        automaticPageTurnActive = false;
        showPendingSyncSaveError();
...
void EpubReaderActivity::silentIndexNextChapterIfNeeded(const uint16_t viewportWidth, const uint16_t viewportHeight) {
  ...
  // FrameBufferLoan returns the renderer storage as a blank frame. Rebuild the
  // page in RAM without touching the panel, which is still showing that page.
...
void EpubReaderActivity::clearWholeBookPaginationState() {
  wholeBookPageCount = 0;
  wholeBookCurrentSpineOffset = 0;
  wholeBookPaginationLayoutSignature = 0;
  wholeBookPaginationSpineIndex = -1;
  wholeBookPaginationExact = false;
}

void EpubReaderActivity::invalidateWholeBookPaginationIndex() {
  resetWholeBookPaginationBuild(true);
  clearWholeBookPaginationState();
  wholeBookPaginationFailedSignature = 0;
  if (!epub) return;

  const std::string indexPath = epub->getCachePath() + WHOLE_BOOK_PAGE_INDEX_FILE_NAME;
  const std::string tmpPath = indexPath + WHOLE_BOOK_PAGE_INDEX_TMP_SUFFIX;
  const std::string backupPath = indexPath + WHOLE_BOOK_PAGE_INDEX_BACKUP_SUFFIX;
  for (const std::string* path : {&indexPath, &tmpPath, &backupPath}) {
    if (Storage.exists(path->c_str()) && !Storage.remove(path->c_str())) {
      LOG_ERR("ERS", "Failed to remove stale whole-book page index: %s", path->c_str());
    }
  }
}

bool EpubReaderActivity::loadWholeBookPaginationIndex(const uint32_t layoutSignature, const int spineIndex) {
  if (!epub || layoutSignature == 0 || spineIndex < 0 || spineIndex >= epub->getSpineItemsCount()) return false;

  const std::string indexPath = epub->getCachePath() + WHOLE_BOOK_PAGE_INDEX_FILE_NAME;
  recoverWholeBookPageIndexBackup(indexPath);

  HalFile file;
  if (!Storage.openFileForRead("ERS", indexPath, file)) return false;
  const auto rejectIndex = [&](const char* reason) {
    file.close();
    LOG_ERR("ERS", "Discarding whole-book page index: %s", reason);
    if (Storage.exists(indexPath.c_str()) && !Storage.remove(indexPath.c_str())) {
      LOG_ERR("ERS", "Failed to remove invalid whole-book page index");
    }
    clearWholeBookPaginationState();
    return false;
  };
  ...
  uint32_t storedTotalPages = 0;
  if (!serialization::tryReadPod(file, magic) || !serialization::tryReadPod(file, version) ||
      !serialization::tryReadPod(file, storedSignature) || !serialization::tryReadPod(file, storedSpineCount) ||
      !serialization::tryReadPod(file, storedTotalPages)) {
    return rejectIndex("truncated header");
  }
  if (magic != WHOLE_BOOK_PAGE_INDEX_MAGIC || version != WHOLE_BOOK_PAGE_INDEX_VERSION) {
    return rejectIndex("format mismatch");
  }
  if (storedSignature != layoutSignature || storedSpineCount != epub->getSpineItemsCount()) {
    return rejectIndex("layout or spine count changed");
  }

  const size_t expectedSize =
      WHOLE_BOOK_PAGE_INDEX_HEADER_SIZE + (static_cast<size_t>(storedSpineCount) + 1U) * sizeof(uint32_t);
  if (file.size() != expectedSize) return rejectIndex("size mismatch");

  uint32_t finalOffset = 0;
  if (!file.seek(WHOLE_BOOK_PAGE_INDEX_HEADER_SIZE + static_cast<size_t>(storedSpineCount) * sizeof(uint32_t)) ||
      !serialization::tryReadPod(file, finalOffset) || finalOffset != storedTotalPages) {
    return rejectIndex("invalid final offset");
  }

  uint32_t spineOffset = 0;
  uint32_t nextSpineOffset = 0;
  if (!file.seek(WHOLE_BOOK_PAGE_INDEX_HEADER_SIZE + static_cast<size_t>(spineIndex) * sizeof(uint32_t)) ||
      !serialization::tryReadPod(file, spineOffset) || !serialization::tryReadPod(file, nextSpineOffset) ||
      spineOffset > nextSpineOffset || nextSpineOffset > storedTotalPages ||
      (section && nextSpineOffset - spineOffset != section->pageCount)) {
    return rejectIndex("invalid spine offset");
  }
  if (!file.close()) LOG_ERR("ERS", "Failed to close whole-book page index after read");

  wholeBookPageCount = storedTotalPages;
  wholeBookCurrentSpineOffset = spineOffset;
  wholeBookPaginationLayoutSignature = layoutSignature;
  wholeBookPaginationSpineIndex = spineIndex;
  wholeBookPaginationExact = true;
  wholeBookPaginationFailedSignature = 0;
  LOG_DBG("ERS", "Whole-book page index loaded: spine=%d offset=%lu total=%lu", spineIndex,
          static_cast<unsigned long>(spineOffset), static_cast<unsigned long>(storedTotalPages));
  return true;
}

void EpubReaderActivity::resetWholeBookPaginationBuild(const bool removeTempFile) {
  if (wholeBookPaginationBuildSection) {
    wholeBookPaginationBuildSection->releaseBuildFile();
    wholeBookPaginationBuildSection.reset();
  }

  wholeBookPaginationBuildState = WholeBookPaginationBuildState::Idle;
  wholeBookPaginationBuildSpec = {};
  wholeBookPaginationBuildSignature = 0;
  wholeBookPaginationBuildTotalPages = 0;
  wholeBookPaginationBuildSpineIndex = 0;
  wholeBookPaginationBuildPausedForLowMemory = false;
  wholeBookPaginationBuildSuppressedImages = false;

  if (!removeTempFile || !epub) return;
  const std::string tmpPath = epub->getCachePath() + WHOLE_BOOK_PAGE_INDEX_FILE_NAME + WHOLE_BOOK_PAGE_INDEX_TMP_SUFFIX;
  if (Storage.exists(tmpPath.c_str()) && !Storage.remove(tmpPath.c_str())) {
    LOG_ERR("ERS", "Failed to remove incomplete whole-book page-index temp file");
  }
}

void EpubReaderActivity::yieldWholeBookPaginationSpineToReader() {
  if (wholeBookPaginationBuildState != WholeBookPaginationBuildState::BuildSpine || !wholeBookPaginationBuildSection ||
      wholeBookPaginationBuildSpineIndex != currentSpineIndex) {
    return;
  }

  LOG_DBG("ERS", "Yielding background whole-book spine %d to foreground reader", wholeBookPaginationBuildSpineIndex);
  wholeBookPaginationBuildSection->releaseBuildFile();
  wholeBookPaginationBuildSection.reset();
  wholeBookPaginationBuildState = WholeBookPaginationBuildState::ScanSpine;
  wholeBookPaginationBuildPausedForLowMemory = false;
}

bool EpubReaderActivity::appendWholeBookPaginationOffset(const uint32_t offset) const {
  if (!epub || wholeBookPaginationBuildSpineIndex < 0) return false;

  const std::string tmpPath = epub->getCachePath() + WHOLE_BOOK_PAGE_INDEX_FILE_NAME + WHOLE_BOOK_PAGE_INDEX_TMP_SUFFIX;
  HalFile output = Storage.open(tmpPath.c_str(), O_RDWR);
  if (!output) {
    LOG_ERR("ERS", "Failed to reopen whole-book page-index temp file");
    return false;
  }

  const size_t expectedSize = WHOLE_BOOK_PAGE_INDEX_HEADER_SIZE +
                              (static_cast<size_t>(wholeBookPaginationBuildSpineIndex) + 1U) * sizeof(uint32_t);
  if (output.size() != expectedSize || !output.seek(expectedSize) || !serialization::tryWritePod(output, offset)) {
    output.close();
    LOG_ERR("ERS", "Failed appending whole-book page offset for spine %d", wholeBookPaginationBuildSpineIndex);
    return false;
  }
  ...
  if (!synced || !closed) {
    LOG_ERR("ERS", "Failed closing whole-book page-index temp file after append");
  }
  return synced && closed;
}

bool EpubReaderActivity::beginWholeBookPaginationBuild(const ReaderRenderSpec& spec, const uint32_t layoutSignature) {
  if (!epub) return false;

  const int spineCount = epub->getSpineItemsCount();
  if (spineCount <= 0 || spineCount > std::numeric_limits<uint16_t>::max()) {
    LOG_ERR("ERS", "Cannot index whole-book pages for invalid spine count: %d", spineCount);
    return false;
  }
  ...
  const std::string tmpPath = epub->getCachePath() + WHOLE_BOOK_PAGE_INDEX_FILE_NAME + WHOLE_BOOK_PAGE_INDEX_TMP_SUFFIX;
  HalFile indexFile;
  if (!Storage.openFileForWrite("ERS", tmpPath, indexFile)) {
    LOG_ERR("ERS", "Failed to create cooperative whole-book page index");
    return false;
  }

  const uint16_t storedSpineCount = static_cast<uint16_t>(spineCount);
  constexpr uint32_t totalPagesPlaceholder = 0;
  constexpr uint32_t firstSpineOffset = 0;
  const bool initialized = serialization::tryWritePod(indexFile, WHOLE_BOOK_PAGE_INDEX_MAGIC) &&
                           serialization::tryWritePod(indexFile, WHOLE_BOOK_PAGE_INDEX_VERSION) &&
                           serialization::tryWritePod(indexFile, layoutSignature) &&
                           serialization::tryWritePod(indexFile, storedSpineCount) &&
                           serialization::tryWritePod(indexFile, totalPagesPlaceholder) &&
                           serialization::tryWritePod(indexFile, firstSpineOffset) && indexFile.sync();
  const bool closed = indexFile.close();
  if (!initialized || !closed) {
    LOG_ERR("ERS", "Failed initializing cooperative whole-book page index");
    Storage.remove(tmpPath.c_str());
    return false;
  }

  wholeBookPaginationBuildSpec = spec;
  wholeBookPaginationBuildSignature = layoutSignature;
  wholeBookPaginationBuildTotalPages = 0;
  wholeBookPaginationBuildSpineIndex = 0;
  wholeBookPaginationBuildPausedForLowMemory = false;
  wholeBookPaginationBuildSuppressedImages = false;
  wholeBookPaginationBuildState = WholeBookPaginationBuildState::ScanSpine;
  wholeBookPaginationFailedSignature = 0;
  LOG_INF("ERS", "Whole-book background pagination started: spines=%d", spineCount);
  return true;
}

void EpubReaderActivity::failWholeBookPaginationBuild(const char* reason) {
  const uint32_t failedSignature = wholeBookPaginationBuildSignature;
  LOG_ERR("ERS", "Whole-book background pagination stopped at spine %d: %s", wholeBookPaginationBuildSpineIndex,
          reason ? reason : "unknown failure");
  resetWholeBookPaginationBuild(true);
  clearWholeBookPaginationState();
  wholeBookPaginationFailedSignature = failedSignature;
}

bool EpubReaderActivity::finishWholeBookPaginationBuild() {
  if (!epub || wholeBookPaginationBuildSignature == 0) return false;

  const int spineCount = epub->getSpineItemsCount();
  if (wholeBookPaginationBuildSpineIndex != spineCount) {
    LOG_ERR("ERS", "Cannot finalize incomplete whole-book index: %d/%d", wholeBookPaginationBuildSpineIndex,
            spineCount);
    return false;
  }
  ...
  const std::string indexPath = epub->getCachePath() + WHOLE_BOOK_PAGE_INDEX_FILE_NAME;
  const std::string tmpPath = indexPath + WHOLE_BOOK_PAGE_INDEX_TMP_SUFFIX;
  HalFile indexFile = Storage.open(tmpPath.c_str(), O_RDWR);
  const size_t expectedSize =
      WHOLE_BOOK_PAGE_INDEX_HEADER_SIZE + (static_cast<size_t>(spineCount) + 1U) * sizeof(uint32_t);
  constexpr size_t totalPagesFieldOffset =
      sizeof(WHOLE_BOOK_PAGE_INDEX_MAGIC) + sizeof(WHOLE_BOOK_PAGE_INDEX_VERSION) + sizeof(uint32_t) + sizeof(uint16_t);
  const bool patched = indexFile && indexFile.size() == expectedSize && indexFile.seek(totalPagesFieldOffset) &&
                       serialization::tryWritePod(indexFile, wholeBookPaginationBuildTotalPages) && indexFile.sync();
  const bool closed = indexFile ? indexFile.close() : true;
  if (!patched || !closed || !promoteWholeBookPageIndex(tmpPath, indexPath)) {
    LOG_ERR("ERS", "Failed finalizing cooperative whole-book page index");
    return false;
  }

  const uint32_t completedSignature = wholeBookPaginationBuildSignature;
  const uint32_t completedPages = wholeBookPaginationBuildTotalPages;
  const bool suppressedImages = wholeBookPaginationBuildSuppressedImages;
  resetWholeBookPaginationBuild(false);
  if (!loadWholeBookPaginationIndex(completedSignature, currentSpineIndex)) {
    LOG_ERR("ERS", "Completed whole-book page index failed read-back validation");
    wholeBookPaginationFailedSignature = completedSignature;
    return false;
  }

  chapterGroupEstimate.valid = false;
  if (suppressedImages) {
    snprintf(APP_STATE.pendingAlertTitle, sizeof(APP_STATE.pendingAlertTitle), "%s", tr(STR_LOW_MEMORY_IMAGES_TITLE));
    snprintf(APP_STATE.pendingAlertBody, sizeof(APP_STATE.pendingAlertBody), "%s", tr(STR_LOW_MEMORY_IMAGES_BODY));
    APP_STATE.pendingAlertGoHomeOnBack.store(false, std::memory_order_relaxed);
    APP_STATE.hasPendingAlert.store(true, std::memory_order_release);
  }
  LOG_INF("ERS", "Whole-book background pagination complete: spines=%d pages=%lu", spineCount,
          static_cast<unsigned long>(completedPages));
  requestUpdate();
  return true;
}

void EpubReaderActivity::armWholeBookPagination(const uint16_t viewportWidth, const uint16_t viewportHeight) {
  if (!epub || !section || activeFootnotePreview || activeSectionLayoutSignature == 0 || currentSpineIndex < 0 ||
      currentSpineIndex >= epub->getSpineItemsCount()) {
    return;
  }

  if (wholeBookPaginationExact && wholeBookPaginationLayoutSignature == activeSectionLayoutSignature &&
      wholeBookPaginationSpineIndex == currentSpineIndex) {
    return;
  }
  if (loadWholeBookPaginationIndex(activeSectionLayoutSignature, currentSpineIndex)) {
    resetWholeBookPaginationBuild(true);
    return;
  }
  if (SETTINGS.indexingMethod != CrossPointSettings::INDEXING_FULL_BOOK) {
    resetWholeBookPaginationBuild(true);
    return;
  }
  ...
  const ReaderRenderSpec spec = readerRenderSpecForProfile(readerFontId, viewportWidth, viewportHeight, profile);
  const uint32_t layoutSignature = readerRenderSpecSignature(spec);
  if (layoutSignature != activeSectionLayoutSignature) {
    LOG_ERR("ERS", "Skipping whole-book pagination because active layout changed (%lu != %lu)",
            static_cast<unsigned long>(layoutSignature), static_cast<unsigned long>(activeSectionLayoutSignature));
    resetWholeBookPaginationBuild(true);
    clearWholeBookPaginationState();
    return;
  }

  if (wholeBookPaginationBuildState != WholeBookPaginationBuildState::Idle) {
    if (wholeBookPaginationBuildSignature == layoutSignature) return;
    resetWholeBookPaginationBuild(true);
  }
  if (wholeBookPaginationFailedSignature == layoutSignature) return;
  if (wholeBookPaginationFailedSignature != 0) {
    wholeBookPaginationFailedSignature = 0;
  }

  if (!beginWholeBookPaginationBuild(spec, layoutSignature)) {
    wholeBookPaginationFailedSignature = layoutSignature;
  }
}

void EpubReaderActivity::tickWholeBookPagination() {
  if (wholeBookPaginationBuildState == WholeBookPaginationBuildState::Idle) return;
  if (SETTINGS.indexingMethod != CrossPointSettings::INDEXING_FULL_BOOK) {
    resetWholeBookPaginationBuild(true);
    return;
  }
  if (!epub || !section || activeFootnotePreview || lastRenderCompleteMs == 0 ||
      backgroundBuildYieldForInput.load(std::memory_order_relaxed) || RenderLock::peek()) {
    return;
  }

  RenderLock lock(*this);
  if (!epub || !section || activeFootnotePreview || backgroundBuildYieldForInput.load(std::memory_order_relaxed) ||
      wholeBookPaginationBuildState == WholeBookPaginationBuildState::Idle ||
      wholeBookPaginationBuildSignature != activeSectionLayoutSignature) {
    return;
  }
  ...
  if (!MemoryBudget::hasHeap(heap, MemoryBudget::EPUB_TEXT_LAYOUT_MIN_FREE,
                             MemoryBudget::EPUB_TEXT_LAYOUT_MIN_MAX_ALLOC)) {
    if (!wholeBookPaginationBuildPausedForLowMemory) {
      LOG_DBG("ERS", "Pausing whole-book background pagination: low heap (free=%u maxAlloc=%u)", heap.freeHeap,
              heap.maxAllocHeap);
    }
    wholeBookPaginationBuildPausedForLowMemory = true;
    return;
  }
  if (wholeBookPaginationBuildPausedForLowMemory) {
    LOG_DBG("ERS", "Resuming whole-book background pagination");
  }
  wholeBookPaginationBuildPausedForLowMemory = false;
  releaseGrayscaleStripScratch();

  const int spineCount = epub->getSpineItemsCount();
  const auto recordCompletedSpine = [this, spineCount](const uint16_t pageCount) {
    if (wholeBookPaginationBuildTotalPages > UINT32_MAX - pageCount) {
      failWholeBookPaginationBuild("page-count overflow");
      return false;
    }
    wholeBookPaginationBuildTotalPages += pageCount;
    if (!appendWholeBookPaginationOffset(wholeBookPaginationBuildTotalPages)) {
      failWholeBookPaginationBuild("page-index write failed");
      return false;
    }

    LOG_DBG("ERS", "Whole-book background spine complete: %d/%d pages=%u total=%lu",
            wholeBookPaginationBuildSpineIndex + 1, spineCount, pageCount,
            static_cast<unsigned long>(wholeBookPaginationBuildTotalPages));
    wholeBookPaginationBuildSpineIndex++;
    wholeBookPaginationBuildState = WholeBookPaginationBuildState::ScanSpine;
    if (wholeBookPaginationBuildSpineIndex == spineCount && !finishWholeBookPaginationBuild()) {
      failWholeBookPaginationBuild("page-index finalization failed");
      return false;
    }
    return true;
  };

  switch (wholeBookPaginationBuildState) {
    case WholeBookPaginationBuildState::Idle:
      return;

    case WholeBookPaginationBuildState::ScanSpine: {
      if (wholeBookPaginationBuildSpineIndex < 0 || wholeBookPaginationBuildSpineIndex >= spineCount) {
        failWholeBookPaginationBuild("invalid spine cursor");
        return;
      }

      if (wholeBookPaginationBuildSpineIndex == currentSpineIndex) {
        // The visible section owns foreground navigation state. Let the normal
        // reader builder complete it one page per loop; never create a second
        // Section for the same cache path.
        if (section->isBuilding()) return;
        if (section->isPartial()) {
          if (partialRebuildStartFailed || partialRebuildAbortedForLowMemory) {
            failWholeBookPaginationBuild("visible section could not finish");
          }
          return;
        }
        recordCompletedSpine(section->pageCount);
        return;
      }

      const EpubRenderMode renderMode = wholeBookPaginationBuildSpec.renderMode;
      // Section must survive across loop ticks while its parser is active, so
      // stack storage is not suitable. Only one fallible instance exists and it
      // is released as soon as this spine finalizes.
      auto cachedSection = makeUniqueNoThrow<Section>(epub, wholeBookPaginationBuildSpineIndex, renderer,
                                                      sectionCacheSuffixForRenderMode(renderMode));
      if (!cachedSection) {
        LOG_ERR("ERS", "OOM allocating background cache reader for spine %d", wholeBookPaginationBuildSpineIndex);
        wholeBookPaginationBuildPausedForLowMemory = true;
        return;
      }

      if (cachedSection->loadSectionFile(wholeBookPaginationBuildSpec) && !cachedSection->isPartial()) {
        const uint16_t pageCount = cachedSection->pageCount;
        cachedSection.reset();
        recordCompletedSpine(pageCount);
        return;
      }

      // Recreate after a partial/miss so no stale SdFat file object survives
      // while the same cache path is opened for incremental output.
      cachedSection.reset();
      wholeBookPaginationBuildSection = makeUniqueNoThrow<Section>(epub, wholeBookPaginationBuildSpineIndex, renderer,
                                                                   sectionCacheSuffixForRenderMode(renderMode));
      if (!wholeBookPaginationBuildSection) {
        LOG_ERR("ERS", "OOM allocating background section builder for spine %d", wholeBookPaginationBuildSpineIndex);
        wholeBookPaginationBuildPausedForLowMemory = true;
        return;
      }
      ...
      bool started = false;
      {
        // The page is already visible on the e-ink panel. Loan the single
        // framebuffer only for parser setup; it is restored before this tick
        // returns and no second screen-sized allocation is introduced.
        GfxRenderer::FrameBufferLoan loan(renderer);
        started = wholeBookPaginationBuildSection->startBuild(wholeBookPaginationBuildSpec);
      }
      if (!started) {
        failWholeBookPaginationBuild("section build start failed");
        return;
      }
      wholeBookPaginationBuildSection->releaseBuildFile();
      wholeBookPaginationBuildState = WholeBookPaginationBuildState::BuildSpine;
      return;
    }

    case WholeBookPaginationBuildState::BuildSpine: {
      if (!wholeBookPaginationBuildSection || !wholeBookPaginationBuildSection->isBuilding()) {
        failWholeBookPaginationBuild("missing active section builder");
        return;
      }
      ...
      {
        GfxRenderer::FrameBufferLoan loan(renderer);
        builtChunk = wholeBookPaginationBuildSection->buildSomeMore(BACKGROUND_BUILD_PAGES_PER_TICK);
      }
      // The parser and writer reopen lazily on the next chunk. Closing both now
      // guarantees that a foreground page render never collides with an SD
      // handle retained by background work.
      wholeBookPaginationBuildSection->releaseBuildFile();
      if (!builtChunk) {
        failWholeBookPaginationBuild("section build failed");
        return;
      }
      if (!wholeBookPaginationBuildSection->isBuildComplete()) return;

      const uint16_t pageCount = wholeBookPaginationBuildSection->pageCount;
      wholeBookPaginationBuildSuppressedImages =
          wholeBookPaginationBuildSuppressedImages || wholeBookPaginationBuildSection->lastBuildImagesWereSuppressed();
      wholeBookPaginationBuildSection.reset();
      recordCompletedSpine(pageCount);
      return;
    }
...
void EpubReaderActivity::renderStatusBar() const {
  ...
  uint32_t wholeBookCurrentPage = 0;
  uint32_t exactWholeBookPageCount = 0;
  if (!activeFootnotePreview && wholeBookPaginationExact &&
      wholeBookPaginationLayoutSignature == activeSectionLayoutSignature &&
      wholeBookPaginationSpineIndex == currentSpineIndex && section->currentPage >= 0) {
    const uint64_t candidate =
        static_cast<uint64_t>(wholeBookCurrentSpineOffset) + static_cast<uint32_t>(section->currentPage) + 1U;
    if (candidate <= wholeBookPageCount) {
      wholeBookCurrentPage = static_cast<uint32_t>(candidate);
      exactWholeBookPageCount = wholeBookPageCount;
    }
  }
  GUI.drawStatusBar(renderer, bookProgress, currentPage, pageCount, title, 0, textYOffset, bookmarked, timeLeft,
                    ...
                    wholeBookCurrentPage, exactWholeBookPageCount);
...
bool EpubReaderActivity::navigateToHref(const std::string& hrefStr, const bool savePosition,
