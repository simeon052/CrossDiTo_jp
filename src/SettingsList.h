...
inline const std::vector<SettingInfo>& getBaseSettingsList() {
  static const std::vector<SettingInfo> baseList = [] {
    ...
    add(SettingInfo::Value(StrId::STR_FRONTLIGHT_SCHEDULE_START, &CrossPointSettings::frontlightScheduleStartQ,
                           {0, CrossPointSettings::FRONTLIGHT_SCHEDULE_SLOT_COUNT - 1, 1}, "frontlightScheduleStartQ",
                           StrId::STR_CAT_DISPLAY));
    add(SettingInfo::Value(StrId::STR_FRONTLIGHT_SCHEDULE_END, &CrossPointSettings::frontlightScheduleEndQ,
                           {0, CrossPointSettings::FRONTLIGHT_SCHEDULE_SLOT_COUNT - 1, 1}, "frontlightScheduleEndQ",
                           StrId::STR_CAT_DISPLAY));
#endif
    ...
    // 組み方向（writingMode）と縦書きの字間（verticalCharSpacing）の設定項目は、
    // まだ出していない。段組みの流し込み（列を右から左へ積む）が未実装で、
    // いま選べるようにしても何も起きないため。docs/japanese.md の Stage 2c 参照。
    add(SettingInfo::Enum(
