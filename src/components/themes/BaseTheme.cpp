...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
  ...
  if (showProgress && (statusBar.showBookProgressPercent || showPageCount || showStablePageNumbers)) {
    ...
    char progressStr[72];
    char pageNumberStr[24];
    if (statusBar.pageCountMode == CrossPointSettings::PAGE_COUNT_BOOK) {
      if (wholeBookCurrentPage > 0 && wholeBookPageCount >= wholeBookCurrentPage) {
        snprintf(pageNumberStr, sizeof(pageNumberStr), "%lu/%lu", static_cast<unsigned long>(wholeBookCurrentPage),
                 static_cast<unsigned long>(wholeBookPageCount));
      } else {
        snprintf(pageNumberStr, sizeof(pageNumberStr), "…/…");
      }
    } else if (pageCountEstimated || pageCount <= 0) {
      // A changing chapter estimate is distracting and the old leading '~'
      // looked as though it belonged to the current page. Keep the exact
      // current page visible and defer the total until pagination is final.
      snprintf(pageNumberStr, sizeof(pageNumberStr), "%d/…", currentPage);
    } else {
      snprintf(pageNumberStr, sizeof(pageNumberStr), "%d/%d", currentPage, pageCount);
    }
    ...
    if (showPageCount && showStablePageNumbers && statusBar.showBookProgressPercent) {
      ...
      snprintf(progressStr, sizeof(progressStr), "%s  %.0f%%", pageNumberStr, bookProgress);
      ...
      snprintf(progressStr, sizeof(progressStr), "%s", pageNumberStr);
    }
