...
constexpr unsigned long IDLE_SAFETY_HEARTBEAT_MS = 1000;

struct FrontlightScheduleRuntime {
  unsigned long lastPollAt = 0;
  uint8_t startSlot = UINT8_MAX;
  uint8_t endSlot = UINT8_MAX;
  uint8_t utcOffsetQ = UINT8_MAX;
...
void applyFrontlightScheduleIfDue() {
  ...
  if (!frontlightScheduleRuntime.initialized || configChanged || crossedBoundary) {
    if (Frontlight.isOn() != shouldBeOn) {
      Frontlight.setOn(shouldBeOn);
      LOG_INF("LIGHT", "Frontlight schedule switched %s", shouldBeOn ? "on" : "off");
    }
    // Keep the persisted-on-next-save setting consistent even when the
    // hardware was already in the scheduled state. The scheduler itself does
    // not write settings at each boundary.
...
unsigned long frontlightScheduleServiceDelayMs(const unsigned long maxDelayMs) {
  if (!Frontlight.present()) return maxDelayMs;
  if (frontlightScheduleRuntime.lastPollAt == 0) return 0;

  const unsigned long now = millis();
  const unsigned long deadline = frontlightScheduleRuntime.lastPollAt + FRONTLIGHT_SCHEDULE_POLL_MS;
  const unsigned long remaining =
      static_cast<int32_t>(now - deadline) >= 0 ? 0UL : static_cast<unsigned long>(deadline - now);
  return remaining < maxDelayMs ? remaining : maxDelayMs;
}
}  // namespace
...
void setup() {
...
#else
  // File transfer still uses the USB serial transport in production. Start it
  // once the former enumeration window has elapsed, without blocking boot.
  scheduleLogSerialTransportStart(250);
#endif
  ...
  if (!powerManager.begin()) {
    ...
    LOG_ERR("MAIN", "Fatal startup failure: power manager unavailable");
    return;
  }
#endif
  halTiltSensor.begin();
  ...
  // Recent books, KOReader credentials, and dictionary validation are loaded
  // by their first consumer. Reader resume no longer pays for unrelated JSON
  // parsing and SD probes before the first page can be shown.
  UITheme::getInstance().reload();
  ButtonNavigator::setMappedInputManager(mappedInputManager);
  logBootHeap("boot state ready");
  // Brightness and warmth always return. Without a valid schedule the on/off
  // state follows Restore Light on Wake; a synced daily schedule takes priority.
  ...
  LOG_DBG("MAIN", "Starting " CROSSDITO_PRODUCT_NAME " version " CROSSINK_VERSION);
  logMemoryStats("Boot");
