...
class CrossPointSettings : public PersistableStore<CrossPointSettings> {
 ...
 public:
  ...
  // Optional fixed daily frontlight window, stored in 15-minute slots. The
  // saved brightness/warmth are reused; no location or network lookup is needed
  // after the device clock has been synchronized.
  static constexpr uint8_t FRONTLIGHT_SCHEDULE_SLOT_COUNT = 96;
  static constexpr uint8_t FRONTLIGHT_SCHEDULE_DEFAULT_START = 19 * 4;
  static constexpr uint8_t FRONTLIGHT_SCHEDULE_DEFAULT_END = 7 * 4;
  uint8_t frontlightScheduleEnabled = 0;
  uint8_t frontlightScheduleStartQ = FRONTLIGHT_SCHEDULE_DEFAULT_START;
  uint8_t frontlightScheduleEndQ = FRONTLIGHT_SCHEDULE_DEFAULT_END;
