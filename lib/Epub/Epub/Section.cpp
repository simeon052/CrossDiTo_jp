...
constexpr uint32_t HEADER_SIZE =
    ...
    sizeof(uint8_t) + sizeof(bool) + sizeof(bool) + sizeof(uint8_t) + sizeof(uint8_t) + sizeof(bool) + sizeof(uint8_t) +
...
uint32_t Section::onPageComplete(std::unique_ptr<Page> page) {
  ...
  const uint32_t serializeStartMs = millis();
  if (!page->serialize(file)) {
    LOG_ERR("SCT", "Failed to serialize page %d", builtPageCount_);
    return 0;
  }
  const uint32_t serializeMs = millis() - serializeStartMs;
  if (serializeMs >= 10) {
    LOG_DBG("SCT", "Serialized page %d in %lu ms", builtPageCount_, static_cast<unsigned long>(serializeMs));
  }
...
bool Section::createSectionFile(const ReaderRenderSpec& spec, const std::function<void()>& popupFn,
  ...
  const auto compiledDir = epub->getCachePath() + "/compiled";
  const std::string compiledCachePath =
      buildOptions.isPreview() ? std::string{} : compiledDir + "/" + std::to_string(spineIndex) + ".bin";
  ...
  // Create cache directory if it doesn't exist
  {
    ...
    if (!compiledCachePath.empty()) Storage.mkdir(compiledDir.c_str());
  }
  ...
  if (!reusedHtml) {
    ...
    for (int attempt = 0; attempt < 3 && !streamed; attempt++) {
      ...
      const uint32_t inflateStartMs = millis();
      streamed = epub->readItemContentsToStream(localPath, tmpHtml, htmlStreamChunkSize);
      const uint32_t inflateMs = millis() - inflateStartMs;
      fileSize = tmpHtml.size();
      // Explicitly close() file before calling Storage.remove()
      tmpHtml.close();
      LOG_DBG("SCT", "HTML inflate: spine=%d attempt=%d ok=%u bytes=%lu chunk=%u time=%lu ms", spineIndex, attempt + 1,
              streamed ? 1U : 0U, static_cast<unsigned long>(fileSize), static_cast<unsigned>(htmlStreamChunkSize),
              static_cast<unsigned long>(inflateMs));
...
bool Section::startBuild(const ReaderRenderSpec& spec, const SectionBuildOptions buildOptions,
  ...
  ctx->parser = makeUniqueNoThrow<ChapterHtmlSlimParser>(
      *epub, ctxPtr->parsePath, renderer, fontId, lineCompression, extraParagraphSpacing, forceParagraphIndents,
      ...
      buildOptions.previewMaxPages, compiledCachePath);
  if (!ctx->parser) {
...
std::unique_ptr<Page> Section::loadPageAt(const int page) const {
  ...
  f.close();
  LOG_DBG("SCT", "Loaded page %d: ok=%u time=%lu ms", page, result ? 1U : 0U,
